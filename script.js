...
// قراءة ملف Excel للأعماق (التعديل هنا)
document.getElementById("excelUpload").addEventListener("change", function(e) {
  ...
  if (file) {
    ...
    reader.onload = function(event) {
      ...
      // تحديث عدد نقاط السمك تلقائياً بناءً على عدد القيم في الملف
      if (depths.length > 0) {
        // تحديث القائمة المنسدلة لتعكس العدد الفعلي
        document.getElementById("depthOption").value = "custom";
        document.getElementById("customDepthValue").value = depths.length;
        document.getElementById("customDepthValue").style.display = "block";
        document.getElementById("depths").value = depths.join(", ");
        document.getElementById("excelFileName").innerText = `✅ تم رفع الملف بنجاح! (${depths.length} قيم)`;
      } else {
        alert("❌ الملف لا يحتوي على قيم صالحة للأعماق!");
...
function calculateOilMass() {
  ...
  let finePerKg = parseFloat(document.getElementById("finePerKg").value);
  ...
  if (!isNaN(finePerKg) && finePerKg > 0) {
    let fine = (parseFloat(mass) * finePerKg).toFixed(2);
    resultHTML += `<br> 💰 <strong>الغرامة:</strong> ${fine} QAR (ريال قطري)`;
  }
